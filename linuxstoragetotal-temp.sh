...
VERSION='linuxstoragetotal.sh--Version-3.36'
# Created; 5/15/2015 by Jim Bodden  (based on linuxstoragecalc.sh Created; 5/15/2014)
# Modified; 7/19/2015 by Jim Bodden
# 
#
#*****************************  NOTES  *****************************
## Due to the way storage is computed with certain tools & OS's this sizes may differ slightly.
## 6/24/2015 -- This script has gone through several modifications in the past as new conditions were encountered.x
...
## 6/26/2015 -- Got dshsphlb working but it broke hhsca4spasoigd3 as the gros drives seems to conflict with enabled status testing
## 6/29/2015 -- Added support for SANTYPE, SANMOD, ASN & LDEV on HITACHI
## 7/18/2015 -- Adding tests & filtering for RAID1 per DIR as of Friday meeting.
#*******************************************************************
...
SAN=0; MPUTIL=0
2>/dev/null rm -f /tmp/*.lst-tmp  ## Delete ALL prior tempfiles from this script
...
if [[ $MIRROR -eq 1 ]]; then echo "This server is using FRAID1 Mirroring" >> /tmp/LOCSTORE.lst-tmp; elif [[ $MIRROR -eq 1 ]]; then echo "This server meets the qualifications but does not appear to be using RAID1 Mirroring" >> /tmp/LOCSTORE.lst-tmp; fi
